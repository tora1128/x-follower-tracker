...
2. Renderにログインします。
3. `SITE_PASSWORD` にサイトを開くための共通パスワードを入れます。
4. `Deploy Blueprint` を押します。
5. デプロイ完了後、Renderで発行されたURLとサイトパスワードを相手に渡します。
...
配布されたURLをスマホで開き、最初にサイトパスワードを入力してください。その後、画面に次の4つを入力します。
